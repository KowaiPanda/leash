...
## 7. What "done" looks like for the demo (5-minute video)

1. Show the x402 v2 spec / Arc Agent Stack docs excerpt that says budget/session
   management is out of scope / "not a sandbox" — this is your problem statement, on screen.
2. Human issues a mandate in the dashboard: scope = `["/api/premium-data"]`,
   ceiling = 2.00 USDC, rate limit = 0.50/hour, expiry = 1 day. Point at the
   Activity Log panel confirming issuance and the HCS sequence number, live.
3. Agent simulator fires 5 requests on the Hedera rail. First few succeed and
   the spend bar moves. Then one is rejected because it would exceed the rate
   limit window — show the 403 reason in the UI, live.
4. Flip the rail toggle to Arc, fire one more approved request — same
   mandate, same guard, now settling USDC on Arc testnet via Circle's Agent
   Stack wallet instead of Hedera. Fire one rejected request on this rail too,
   to show the guard doesn't care which rail asked.
5. Open the Audit Trail page — show the full flattened event feed across
   both rails, then open the mirror node link to show the same events
   independently timestamped and sequence-numbered by Hedera itself.
6. Try to raise the ceiling — narrate that it requires signatures from two
   separately-held Authorization Keys to meet the quorum threshold before
   Privy will accept the update at all; show the Activity Log confirming the
   new ceiling once both signatures are supplied.